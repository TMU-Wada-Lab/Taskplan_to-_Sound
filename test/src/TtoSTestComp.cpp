...
{
  TtoSTestInit(manager);
  TtoSInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("TtoSTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("TtoSTest0");
  if (comp == nullptr)
  {
  ...
  TtoSTest* testcomp = dynamic_cast<TtoSTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
